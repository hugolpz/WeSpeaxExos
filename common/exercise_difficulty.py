...
class Difficulty:
    ...
    def find_all_scores(self):
        ...
        word_exo_df["Full_sentence"].apply(lambda text: self.get_token_list(text))
        sent_exo_df["Full_sentence"].apply(lambda text: self.get_token_list(text))
        ...
        self.word_diff_df[['length','zipf_freq','score']] = self.word_diff_df["word"].apply(lambda text: self.find_word_difficulty(text))
        ...
        self.word_diff_df["boxcox_score"] = self.word_diff_df["score"].apply(lambda score: self.transform_difficulty(score))

        # get the level of the transformed difficulty score
        quantile_ranks = self.word_diff_df["boxcox_score"].apply(lambda score: self.find_difficulty_quantiles(score))
        self.word_diff_df["Difficulty"] = self.find_difficulty_level(quantile_ranks)

        # set the difficulty for the word exercises
        word_exo_df[["Score_target_word", "Difficulty"]] = word_exo_df["Full_sentence"].apply(lambda text: self.set_word_difficulty(text))
        word_exo_df = word_exo_df.merge(self.word_diff_df[["boxcox_score", "Difficulty"]], how = "left", left_on = "Full_sentence", right_on = "word").drop(columns = ["word"])

        # get the average sentence length for each full sentence in the exercise dataset
        sent_exo_df["Length_sentence"] = sent_exo_df["Full_sentence"].apply(lambda text: self.avg_sent_length(text))
        
        # get length of right answers i.e. target words
        sent_exo_df["Length_traget_word"] = sent_exo_df["Full_sentence"].apply(lambda x: len(x))
        
        # get length of the longest word
        sent_exo_df["Length_longest_word"] = sent_exo_df["Full_sentence"].apply(lambda text: self.find_wLengthMax(text))
        
        # get the difficulty score of the right answers (target words), rarest word in the sentence and the sentence
        sent_exo_df["Score_target_word"] = sent_exo_df["Right_answer"].apply(lambda text: self.set_word_difficulty(text)[0])
        sent_exo_df["Score_rarest_word"] = sent_exo_df["Right_answer"].apply(lambda text: self.find_wSRarest(text))
        sent_exo_df["Score_sentence"] = sent_exo_df["Right_answer"].apply(lambda text: self.find_SScore(text))
        ...
        quantile_ranks = sent_exo_df["Score_sentence"].apply(lambda score: self.find_difficulty_quantiles(score))
